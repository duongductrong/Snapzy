...
- **Sweep resolution**: the band of offsets the frames confirm is only a few pixels wide around a true offset, so the sweep steps by one pixel across the range the scroll plausibly travelled (half to twice the expected step) and coarsely elsewhere. A coarse-only sweep walks straight past the answer.
