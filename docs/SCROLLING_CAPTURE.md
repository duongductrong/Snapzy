...
- **Offset verification** (`ScrollingCaptureOffsetVerifier.swift`) decides which offset is real; the band scores only propose one. Row `y` of the current frame must equal row `y + offset` of the previous one across the whole overlap, judging only rows that carry contrast (at least 50% must agree, `maximumMeanDifference = 14`). This is what a flat page needs: background agrees at every offset, so the sampled bands tie and the scan settles on whichever candidate it saw first. When the scan's answer is rejected, the plausible range is swept for offsets the frames do confirm, grouped into runs. One run is taken as the answer; separated runs are separated by Vision, since a repeating layout confirms a wrong repeat as readily as the true offset. An overlap with too little content to judge (`noEvidence`) falls back to the old score gates.
- **Plausible travel**: an offset shorter than the step the scroll was asked to travel is an intermediate frame caught mid-scroll and is refused, because committing it pins later steps to the wrong overlap. A longer one is accepted when Vision confirms it, since frames do get missed. Settled partial steps at the end of a scroll area come in through `allowsSettledPartialStep`, which lowers the expected step to what Vision measured.
