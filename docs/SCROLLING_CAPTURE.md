...
- **Offset verification** (`ScrollingCaptureOffsetVerifier.swift`) decides which offset is real; the band scores only propose one. Row `y` of the current frame must equal row `y + offset` of the previous one across the whole overlap, judging only rows that carry contrast (at least 50% must agree, `maximumMeanDifference = 10`). This is what a flat page needs: background agrees at every offset, so the sampled bands tie and the scan settles on whichever candidate it saw first. When the scan's answer is rejected, the plausible range is swept for offsets the frames do confirm, grouped into runs. One run is taken as the answer; separated runs are separated by Vision, since a repeating layout confirms a wrong repeat as readily as the true offset. An overlap with too little content to judge (`noEvidence`) falls back to the old score gates.
- **Plausible travel**: a page does not always travel the whole step it is asked to, so a confirmed offset is committed down to a fifth short of the request, and up to twice it when frames were missed (Vision must confirm the longer one). A move far shorter than the step is refused: an intermediate frame caught mid-scroll looks exactly like it, and a repeating layout confirms such an offset as readily as the real one. A genuine short travel — at the end of a scroll area, say — lands through `allowsSettledPartialStep`, which lowers the expected step to what Vision measured on a viewport independently verified as settled.
