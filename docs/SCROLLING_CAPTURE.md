...
- **Plausible travel**: a page does not always travel the whole step it is asked to, so a confirmed offset is committed down to a third short of the request, and up to twice it when frames were missed (Vision must confirm the longer one). A move far shorter than the step is refused: an intermediate frame caught mid-scroll looks exactly like it, and a repeating layout confirms such an offset as readily as the real one. A genuine short travel — at the end of a scroll area, say — lands through `allowsSettledPartialStep`, which lowers the expected step to what Vision measured on a viewport independently verified as settled.
