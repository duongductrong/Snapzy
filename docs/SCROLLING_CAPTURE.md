...
- **Offset verification** (`ScrollingCaptureOffsetVerifier.swift`): score-based matching cannot separate offsets where the page is mostly flat, because background agrees at every offset, the sampled bands tie, and the best candidate is then rejected as ambiguous. When scoring ties or rejects, each candidate is checked against the frames themselves: row `y` of the current frame must equal row `y + offset` of the previous one across the whole overlap, judging only rows that carry contrast (at least 50% of them must agree). If no candidate holds up, the plausible range is swept with the verifier. A single confirmed cluster that also agrees with Vision is accepted; anything wider stays ambiguous.
