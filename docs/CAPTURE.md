...
- Delayed capture (`delayedCapture`, unbound by default; menu bar item; `snapzy://capture/delayed`): `ScreenCaptureViewModel.captureDelayed()` shows a non-activating countdown HUD (`CaptureDelayCountdownController`, length from `screenshot.delayedCaptureSeconds`: 3/5/10 s, default 3) that never takes focus, so menus and hover states opened during the countdown stay open. Esc or clicking the HUD cancels. When it ends the HUD is hidden and area selection starts on a frozen snapshot regardless of the Freeze screen preference (`forceFrozenSelection`), so the transient UI is preserved.
...
- Interaction modes (`AreaSelectionInteractionMode`): `.manualRegion` and `.applicationWindow`, toggled at runtime via the configurable overlay key. Recording sessions add `.fullDisplay` (Enter), see [RECORDING.md](RECORDING.md#picking-a-target-in-the-recording-overlay). Results flow as `AreaSelectionResult{target: .rect|.window|.display, displayID, mode, displayIDs, spansMultipleDisplays}`; `.display` is recording-only, and screenshot consumers treat it like `.rect`.
