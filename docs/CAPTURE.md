...
- Screenshot outputs use each display's **native** pixel density. Non-Retina external monitors save at 1× (matching the system screenshot tool; see issue #414). When a selection spans Retina and non-Retina displays, the composite crop promotes and edge-enhances each low-density display slice up to the highest native scale in the selection before drawing it into the shared canvas, preserving native Retina slices without re-sharpening them.
...
- Capture Subject (`objectCutout`, menu title **Capture Subject**, default `⌘⇧1`) is a standalone PixelSnap-style overlay. Drag a rectangle around an object; on mouse-up Snapzy locks a snapped preview (`PixelObjectSnapper` flood-fills the search corners and tightens to the remaining pixels). Capture does **not** happen at this step. The overlay never dims the rest of the screen — it marks the search/preview rect with a cyan PixelSnap-style border (plus a thin inner white stroke). Recording keeps its own independent dim preference.
- Click the camera button in the locked preview to commit. Hovering the camera does not capture. Clicking outside the preview unlocks it and lets you drag again. `Escape` dismisses a locked preview first, then cancels the session. A click or Return on the preview itself does not save.
- The overlay captures a live display crop for the locked preview (`CaptureSubjectPreviewCapturer` + `CGDisplayCreateImage`). Overlay panels use `sharingType = .readOnly` so remote-display clients can show them; the controller briefly hides the panels around its own preview snapshot. Source app context is captured when the session starts, before Snapzy becomes frontmost.
- After the camera click, `ScreenCaptureViewModel.finishObjectCutout(from:)` runs `ForegroundCutoutService` (macOS 14+, Vision `VNGenerateForegroundInstanceMaskRequest`, all subject instances masked on the full canvas). If Vision finds no subject, Capture Subject keeps the locked preview pixels (`fallbackToOriginalWhenNoSubject`, decision `.skippedNoSubjectFallback`) instead of showing “No subject detected”. Annotate Remove Background still surfaces that failure. Fully on-device.
- `PixelObjectSnapper` flood-fills from the four search corners with a high per-channel color tolerance (112) so noisy or slightly shaded backgrounds still peel away. If remaining content covers ≥99.5% of the search rect, the original drag is kept.
