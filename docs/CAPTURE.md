...
- Models are managed under Settings → Capture → OCR → OCR Model (see [`PREFERENCES.md`](PREFERENCES.md)). `OCRModelCatalog` merges the bundled `Resources/ocr-model-catalog.yaml` defaults with validated user definitions from `ocr.userCatalogModels`. The bundled catalog ships empty, so in a stock build every downloadable entry comes from the user. User definitions can be entered in the UI or imported/exported as strict JSON/YAML/YML, with direct HTTPS or Hugging Face artifact sources. The runtime adapter is allow-listed to `ppocr-db-ctc-v1`, artifact names remain the fixed `det.onnx` / `rec.onnx` / `dict.txt` contract, and ONNX SHA-256 checksums are required and verified. Metadata import never fetches or executes a model; Download remains an explicit action.
- Downloaded model binaries install into `Application Support/Snapzy/OCRModels/<id>/` and are never included in config/manifest exports. Editing, replacing, or deleting a user definition invalidates stale installed files and unloads its cached session.
