...
- `AreaSelectionWindow` is a nonactivating `NSPanel` per display with `sharingType = .readOnly` so external display and remote-desktop clients can render the selection chrome; Snapzy temporarily hides the relevant overlay around its own snapshots so it never bakes into captures. `collectionBehavior = [.canJoinAllSpaces, .fullScreenAuxiliary, .stationary]` joins every Space at `.screenSaver` level (membership loss is the watchdog's ground-truth check above); a pointer-tracking timer promotes "key-follows-pointer" keyboard ownership for non-activated live sessions. In live passthrough sessions (below) the timer stays off — Esc/arrows arrive via the event tap regardless of which window is key, and Space/application-toggle keys are passed through to the key overlay.
...
- During a Smart Element session, `SmartElementCaptureController` owns one live, externally shareable overlay panel per screen. `SmartElementWindowOwnerResolver` finds the topmost non-Snapzy window under the cursor, `SmartElementQueryService` uses the window PID with `AXUIElementCopyElementAtPosition`, and `AXElementInspector` normalizes AX bounds into AppKit bottom-left coordinates for highlight rendering. To ensure 60fps overlay performance, the AX queries run on a background queue with a throttle, preventing main thread blocking. The controller dismisses every overlay before committing the capture.
