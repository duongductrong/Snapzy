...
- Models are managed under Settings → Capture → OCR → OCR Model (see [`PREFERENCES.md`](PREFERENCES.md)). `OCRModelCatalog` merges the signed `Resources/ocr-model-catalog.yaml` defaults with validated user definitions from `ocr.userCatalogModels`. User definitions can be entered in the UI or imported/exported as strict JSON/YAML/YML, with direct HTTPS or Hugging Face artifact sources. The runtime adapter is allow-listed to `ppocr-db-ctc-v1`, artifact names remain the fixed `det.onnx` / `rec.onnx` / `dict.txt` contract, and ONNX SHA-256 checksums are required and verified. Metadata import never fetches or executes a model; Download remains an explicit action.
- PP-OCR binaries install into `Application Support/Snapzy/OCRModels/<id>/` and are never included in config/manifest exports. The bundled Tiny/Small/Medium metadata is YAML-backed but its ids, URLs, sizes, and hashes remain unchanged, so existing installations require no migration. Editing, replacing, or deleting a user definition invalidates stale installed files and unloads its cached session.
- Remote endpoints remain separate from downloadable definitions: endpoint metadata is JSON-persisted in `ocr.customModels`; optional API keys live in the macOS Keychain (service `com.trongduong.snapzy.ocr`) and are never exported.
- Only one model is active, and uninstalled models are not selectable. An unavailable or removed selection resolves to built-in Vision and is persisted back (`OCRModelResolver`); at launch `OCRModelStore.validateInstalledModelsOnLaunch()` prunes persisted installs whose files are missing and resets a selection that referenced one. Catalog load failure skips that destructive prune, preserving existing installed ids until catalog authority is restored. If model files disappear mid-session, the router catches `PPOCRError.modelFilesMissing`, marks the model not-installed (`OCRModelStore.markMissing`), persists built-in, and retries recognition via Vision.
...
| `Snapzy/Services/Media/OCR/` | `OCRProvider` engines (Vision, PP-OCR det+rec on ONNX Runtime, remote OpenAI-compatible), bundled YAML + user JSON/YAML model catalogs, download/install state, custom endpoint persistence |
