...
- **OCR pane**:
  - OCR Model: active recognition model (`PreferencesOCRModelListView` + `Models/PreferencesOCRModelListViewModel`; selection persisted in `ocr.selectedModel` as `builtin` | `dl:<id>` | `custom:<uuid>`).
    ...
    - The bundled downloadable catalog is loaded from `Resources/ocr-model-catalog.yaml`, not hard-coded Swift. It currently defines PP-OCRv6 Tiny/Small/Medium; binaries still install on demand into `Application Support/Snapzy/OCRModels/<id>/`, so the default app size is unchanged.
